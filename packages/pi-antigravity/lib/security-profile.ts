...
const BROKER_INSTRUCTIONS = [
  ...
  "Native tools such as view_file may still appear in the catalog. They cannot execute: calling one terminates this conversation immediately and returns no file content.",
  "Use only MCP tools whose names start with `pi__`; Pi owns permissions and executes every real tool.",
  "Pi read requires the argument `path`, not file_path, filePath, or AbsolutePath.",
